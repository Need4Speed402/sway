...
static void pretty_print_output(json_object *o) {
	...
	json_object *make, *model, *serial, *scale, *scale_filter, *subpixel,
		*transform, *max_render_time, *adaptive_sync_enabled;
	...
	json_object_object_get_ex(o, "adaptive_sync_enabled", &adaptive_sync_enabled);
	json_object *x, *y;
	...
	if (json_object_get_boolean(non_desktop)) {
		...
		printf(
			...
			json_object_get_boolean(adaptive_sync_enabled) ? "on" : "off"
		);
