...
struct sway_container *container_create(struct sway_view *view) {
	...
	c->title_bar.tree = alloc_scene_tree(c->scene_tree, &alloc_failure);
	c->title_bar.border = alloc_scene_tree(c->title_bar.tree, &alloc_failure);
	c->title_bar.background = alloc_scene_tree(c->title_bar.tree, &alloc_failure);
	...
	for (int i = 0; i < 4; i++) {
		alloc_rect_node(c->title_bar.border, &alloc_failure);
	}

	for (int i = 0; i < 5; i++) {
		alloc_rect_node(c->title_bar.background, &alloc_failure);
	}

	c->border.tree = alloc_scene_tree(c->scene_tree, &alloc_failure);
	c->content_tree = alloc_scene_tree(c->border.tree, &alloc_failure);
	...
	if (!alloc_failure && !scene_descriptor_assign(&c->scene_tree->node,
